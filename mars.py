...
for ele in dates: #Looping over dates
    ...
    eclipitic_pos_mars = matrix_prod(R_euler_mars,pos_mars) #Applying euler rotation
    ...
    r_eclip = sub_vec(eclipitic_pos_mars,eclipitic_pos_earth) #FInding geo-centric position of Mars 
...
plt.plot(time_scale[0:50],ra_alg[0:50],color = 'y', label = 'Prediction from algorithm')
plt.plot(time_scale[50:],ra_alg[50:],color = 'y')
plt.grid()
